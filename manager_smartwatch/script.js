...
// Tham chiếu đến collection smartwatch
const productsRef = collection(db, "smart watch");
...
onSnapshot(productsRef, (snapshot) => {
    ...
    snapshot.forEach((doc) => {
        ...
        li.innerHTML = `
                    Thương hiệu: ${data.brand || "Chưa xác định"}, Mô tả: ${data.describe || "Chưa xác định"}, Tên: ${data.name || "Chưa xác định"}, Giá: ${data.price || 0} VND
                    <button onclick="editProduct('${doc.id}', '${data.brand || ''}', '${data.describe || ''}', '${data.name || ''}', ${data.price || 0})">Sửa</button>
...
document.getElementById("productForm").addEventListener("submit", async (e) => {
    ...
    try {
        if (productId) {
            // Sửa sản phẩm
            const productRef = doc(db, "smart watch", productId);
            await updateDoc(productRef, { brand, describe, name, price });
            alert("Đã cập nhật sản phẩm!");
        } else {
            // Thêm sản phẩm mới
            await addDoc(productsRef, { brand, describe, name, price });
...
window.editProduct = (id, brand, describe, name, price) => {
...
window.deleteProduct = async (id) => {
    if (confirm("Bạn có chắc muốn xóa sản phẩm này?")) {
        try {
            const productRef = doc(db, "smart watch", id);
