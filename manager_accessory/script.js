...
// Tham chiếu đến collection Accessories
const productsRef = collection(db, "accessory");
...
onSnapshot(productsRef, (snapshot) => {
    ...
    snapshot.forEach((doc) => {
        ...
        li.innerHTML = `
                Mô tả: ${data.describe || "Chưa xác định"}, Thương hiệu: ${data.brand || "Chưa xác định"}, Tên: ${data.name || "Chưa xác định"}, Giá: ${data.price || 0} VND
                <button onclick="editProduct('${doc.id}', '${data.describe || ''}', '${data.brand || ''}', '${data.name || ''}', ${data.price || 0})">Sửa</button>
...
document.getElementById("productForm").addEventListener("submit", async (e) => {
    ...
    try {
        if (productId) {
            // Sửa sản phẩm
            const productRef = doc(db, "accessory", productId);
            await updateDoc(productRef, { describe, brand, name, price });
            alert("Đã cập nhật sản phẩm!");
        } else {
            // Thêm sản phẩm mới
            await addDoc(productsRef, { describe, brand, name, price });
...
window.editProduct = (id, describe, brand, name, price) => {
...
window.deleteProduct = async (id) => {
    if (confirm("Bạn có chắc muốn xóa sản phẩm này?")) {
        try {
            const productRef = doc(db, "accessory", id);
